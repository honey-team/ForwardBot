...
@app_commands.describe(show_original='Whether to show the original message link. Might be needed to set to off on servers with strict automod.', anonymous='Whether to send the message anonymously. (in test)')
@app_commands.user_install()
async def send(ctx: discord.Interaction, show_original: bool=True, anonymous: bool=False):
    ...
    if anonymous:
        await ctx.response.send_message('shh...', ephemeral=True)
